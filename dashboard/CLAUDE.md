# CLAUDE.md

이 프로젝트는 **File-Based Planning Workflow**를 따릅니다.

참고: <https://github.com/ahastudio/file-based-planning-workflow>

---

## 작업 흐름

문서는 다음 순서로 작성하고, 각 단계마다 사람이 검토한 후 다음 단계로 진행합니다.

```
README → (사람 검토) → spec → (사람 검토) → plan → (사람 검토) → tasks + progress + findings
```

### 단계별 설명

1. **README.md** — 기능 개요, 배경, 목표, 동작 방식을 간단히 서술
2. **spec.md** — PRD 수준의 요구사항 (유저 스토리, 인수 시나리오, 기능 요구사항, 제약사항)
3. **plan.md** — 기술 구현 계획 (아키텍처, 구현 단계, 검증 방법)
4. **tasks.md** — 구체적인 작업 항목과 체크리스트
5. **progress.md** — 세션별 진행 로그, 테스트 결과, 오류 기록
6. **findings.md** — 기술적 발견, 중요 결정, 배운 점 (언제든지 즉시 업데이트)

---

## 규칙

### 문서 관리

- 각 단계 문서를 작성한 뒤 **반드시 사람 검토를 기다린다.**
- 사람이 승인하기 전에 다음 단계 문서를 작성하지 않는다.
- `findings.md`는 예외로, 배운 점이 생기면 **즉시** 기록한다.
- 문서를 자주 업데이트하고 동기화한다.
- 사고 과정을 끊임없이 기록한다.

### 커밋

- 문서나 코드가 의미 있는 상태에 도달할 때마다 **자주 커밋**한다.
- 커밋 메시지는 변경 내용을 명확히 설명한다.

### 구현

- 구현은 `plan.md`가 승인된 이후에 시작한다.
- 구현 중 발견한 사항은 `findings.md`에 즉시 기록한다.
- 진행 상황은 `progress.md`에 세션 단위로 기록한다.
